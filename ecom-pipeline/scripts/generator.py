...
def seed_products(cur, n=80):
    ...
    for _ in range(n):
        pid = str(uuid.uuid4())
        cost = round(random.uniform(15, 800), 2)   # Python float
        sell = round(cost * random.uniform(1.3, 2.5), 2)  # Python float
        cur.execute(
            """
        ...
        # FIX: explicitly cast to float — guards against any Decimal leakage
...
def generate_daily_data(num_orders: int = 250):
    ...
    # FIX: psycopg2 maps NUMERIC/DECIMAL → decimal.Decimal, not float.
    # Always cast explicitly to avoid TypeError when doing float arithmetic.
    products = {r[0]: float(r[1]) for r in product_rows}
    ...
    for _ in range(num_orders):
        ...
        # FIX: defensive float() cast — safe regardless of where price came from
        # (DB fetch path or seed_products path)
